...
class Object_Detector:
    def __init__(self, opt):
        self.opt = opt
        self.pub = rospy.Publisher('/cd70/camera_node/image/detected', Image, queue_size=10)
    ...
    def callback(self, data):
        ...
        with torch.no_grad():
            if opt.update:  # update all models (to fix SourceChangeWarning)
                for opt.weights in ['yolov5s.pt', 'yolov5m.pt', 'yolov5l.pt', 'yolov5x.pt', '../runs/train/yolov5s_results/weights/best.pt']:
                    im0 = detect(opt)
                    strip_optimizer(opt.weights)
            else:
                im0 = detect(opt)
    ...
    def subscriber(self):
        rospy.Subscriber("/cd70/camera_node/image/raw", Image, self.callback)
